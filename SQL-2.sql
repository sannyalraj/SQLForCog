...
select distinct department from worker;
 -- use group by 
 select worker group by department;
...
(select salary from worker order by salary desc limit 3) order by salary;
...
3>=(select count(distinct salary) from worker w2 where w1.salary>=w2.salary) order by w1.salary desc;

-- Q-48. Write an SQL query to fetch nth max salaries from a table.
select salary from worker w1 where
n>=(select count(distinct salary) from worker w2  where w1.salary<=w2.salary) order by w1.salary desc;
